...
export async function getUpcoming(type,db=null){
 if(type==='event')return (await events()).slice(0,10).concat(dbFallback(db,type)).slice(0,10);
 if(type==='theater'||type==='setlist')return (await theater()).filter(x=>type==='theater'||x.setlist).slice(0,10).concat(dbFallback(db,type)).slice(0,10);
 if(type==='birthday')return (await birthdays({days:30})).slice(0,10);
 if(type==='live')return (await allLive()).slice(0,10);
 if(type==='graduation')return (await news({category:'Graduation'})).slice(0,10).concat(dbFallback(db,type)).slice(0,10);
 if(type==='songs')return (await news({category:'Music'})).slice(0,10).concat(dbFallback(db,type)).slice(0,10);
 return [];
}
export async function getLatest(type,db=null){
 if(type==='event'){const api=await events({status:'completed'});return (api.length?api:dbFallback(db,type,true)).slice().reverse().slice(0,10)}
 if(type==='theater'||type==='setlist'){const api=(await theater({status:'completed'})).filter(x=>type==='theater'||x.setlist);return (api.length?api:dbFallback(db,type,true)).slice().reverse().slice(0,10)}
 ...
 if(type==='graduation'){const api=await news({category:'Graduation'});return (api.length?api:dbFallback(db,type,true)).slice(0,10)}
