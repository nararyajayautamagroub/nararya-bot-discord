...
export function createExtendedFeatures({db,client,embed,botControl}={}){
 ...
 async function handle(i){
  ...
  if(n==="community"){
   ...
   if(s==="poll"){const q=i.options.getString("question",true),o=i.options.getString("options",true).split(/[|,]+/).map(x=>x.trim()).filter(Boolean).slice(0,8);if(o.length<2)return i.reply({embeds:[embed("Poll","Minimal 2 pilihan.",{color:0xEF4444})],ephemeral:true}),true;return i.reply({embeds:[embed("Poll",q+"\n"+o.map((x,n)=>(n+1)+". "+x).join("\n"),{color:0x3B82F6})]}),true;}
   if(s==="star"){return i.reply({embeds:[embed("Starboard","Message "+i.options.getString("message_id",true)+" • Stars "+i.options.getInteger("stars",true),{color:0xFBBF24})]}),true;}
  }
  return false;
 }
