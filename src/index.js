...
client.on('interactionCreate',async i=>{
 try{
  ...
  if(n==='sim'){
   ...
   if(sub==='daily'){if(Date.now()-p.daily_at<86400000)return i.reply({content:'Daily reward sudah diambil hari ini.',ephemeral:true});db.prepare('UPDATE tycoon SET money=money+50000,daily_at=? WHERE guild_id=? AND user_id=?').run(Date.now(),gid,uid);return i.reply({embeds:[embed('🎁 Daily Reward','Kamu mendapatkan **Rp50.000**!',{color:EMBED_COLORS.success})]})}
