...
const embed=(title,description='',opts={})=>{const e=new EmbedBuilder().setTitle(title).setDescription(description).setColor(opts.color??EMBED_COLORS.default).setTimestamp().setAuthor({name:'BOT NARARYA GROUB'}).setFooter({text:'PT. NEXOVONARSACORPORATION - All Right Reserved'});if(opts.url)e.setURL(opts.url);if(opts.image)e.setImage(opts.image);if(opts.thumbnail)e.setThumbnail(opts.thumbnail);if(opts.fields)e.addFields(opts.fields);return e};
