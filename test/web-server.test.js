...
test("website runtime wires health, auth, cookies, and CSP",async()=>{
  const previous={host:process.env.WEBSITE_HOST,port:process.env.WEBSITE_PORT,disabled:process.env.WEBSITE_DISABLED};
  ...
  try{
    ...
    db.close();
    if(previous.host===undefined)delete process.env.WEBSITE_HOST;else process.env.WEBSITE_HOST=previous.host;
    if(previous.port===undefined)delete process.env.WEBSITE_PORT;else process.env.WEBSITE_PORT=previous.port;
    if(previous.disabled===undefined)delete process.env.WEBSITE_DISABLED;else process.env.WEBSITE_DISABLED=previous.disabled;
  }
});
